...
puts "Il y a #{journalistes.size} handle." 

puts ">Combien d'handle contiennent un numéro ?"
puts "Il y a #{journalistes.count{ |x| x[/\d/]}} handle contenant des nombres." #le "d" est un raccourci pour trouver les chiffres dans la liste

puts ">Combien d'handle contiennent les 4 lettres du prénom Aude à la suite (sans prendre en compte les majuscules) ?"
puts "Il y a #{journalistes.count{ |x| x[/aude/i]}} handle contenant Aude." #le i permet de prendre toutes les lettres sans faire de différence entre les maj et minuscules


puts "Combien commencent par une majuscule (première lettre juste après le @) ?"
print ">"
puts journalistes.count { |x| 
y=x.slice(1)    # slice(1) = ne prend en compte que la 2ème lettre de chaque valeur
  y[/[A-Z]/]
}
...
table_sorted = journalistes.sort_by!{|x| x.length} #classe du plus petit au plus grand

y = 0
loop do
    puts table_sorted[y] 
    y += 1
    if y == 51 #affiche du plus petit au plus grand tant que la 50ème valeur est pas sortie
      break
    end
  end

puts "#{journalistes.index("@epenser")} est la position de e-penser dans la liste."